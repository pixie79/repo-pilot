...
/// Render one frame without a terminal, which is the only practical way to
/// review a TUI from a script. `html` keeps the colours, for the screenshot in
/// the README; `text` keeps only the layout, which is what a diff wants.
pub async fn snapshot(width: u16, height: u16, view: &str, html: bool) -> Result<String> {
...
/// One dashboard frame as a standalone HTML page, styles and all.
///
/// [`render_once`] keeps each cell's symbol and throws its style away, which is
/// what a layout check wants and exactly wrong for a screenshot: this tool's
/// whole claim is that colour carries the state. Rendering from the same
/// `TestBackend` buffer means the picture is generated by the real UI code
/// rather than reconstructed by scraping a terminal, so it cannot drift from
/// what the dashboard actually draws.
fn render_html(app: &App, width: u16, height: u16) -> Result<String> {
    ...
    for y in 0..height {
        // Coalesce runs of identically-styled cells, so a row is a handful of
        ...
        for x in 0..width {
            ...
            let style = css_for(cell);
            if Some(&style) != run_style.as_ref() {
                flush_span(&mut body, &run, run_style.as_deref());
                run.clear();
    ...
    Ok(format!(
        "<!doctype html>\n<meta charset=\"utf-8\">\n<title>repo-pilot</title>\n\
         <style>\n\
         html,body{{margin:0;padding:0;background:{BG}}}\n\
         pre{{margin:0;padding:20px;background:{BG};color:{FG};\n\
         font:15px/1.32 'SF Mono','Menlo','DejaVu Sans Mono',monospace;\n\
...
fn flush_span(out: &mut String, run: &str, style: Option<&str>) {
    if run.is_empty() {
        return;
    }
    match style {
...
/// The page's own foreground and background, standing in for `Color::Reset`.
const FG: &str = "#c8ccd4";
const BG: &str = "#14161c";

fn css_for(cell: &ratatui::buffer::Cell) -> String {
    use ratatui::style::Modifier;
    let m = cell.modifier;
    // REVERSED swaps the pair, and an unset side reverses to the page default.
    let (fg, bg) = if m.contains(Modifier::REVERSED) {
        (css_color(cell.bg, BG), css_color(cell.fg, FG))
    } else {
        (css_color(cell.fg, FG), css_color(cell.bg, BG))
    };
...
/// A terminal palette for the sixteen named colours. Chosen to sit close to a
/// dark default scheme, since that is what the dashboard is designed against.
fn css_color(color: ratatui::style::Color, reset: &str) -> String {
    use ratatui::style::Color;
    match color {
        Color::Reset => reset.to_string(),
        Color::Black => "#14161c".into(),
        Color::Red => "#e05561".into(),
        Color::Green => "#8cc265".into(),
        Color::Yellow => "#d5a253".into(),
        Color::Blue => "#5c9bd6".into(),
        Color::Magenta => "#bf68d9".into(),
        Color::Cyan => "#42b3c2".into(),
        Color::Gray => "#a6accd".into(),
        Color::DarkGray => "#6b7089".into(),
        Color::LightRed => "#ff6a75".into(),
        Color::LightGreen => "#a5e075".into(),
        Color::LightYellow => "#f0c674".into(),
        Color::LightBlue => "#79b8ff".into(),
        Color::LightMagenta => "#d38aea".into(),
        Color::LightCyan => "#5fd7e8".into(),
        Color::White => "#e6e9ef".into(),
        Color::Rgb(r, g, b) => format!("#{r:02x}{g:02x}{b:02x}"),
        // Only the 16 named slots are used by this UI; anything indexed is a
        // future addition, and grey is a visible "look at me" rather than a
        // wrong colour confidently rendered.
        Color::Indexed(_) => "#8b90a0".into(),
    }
}
